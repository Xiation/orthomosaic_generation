...
class Combiner:
    ...
    def combine(self, index2):
        ...
        # create binary mask for new warped image
        gray_warped_img2 = cv2.cvtColor(warpedImage2, cv2.COLOR_BGR2GRAY)
        _, mask_binary = cv2.threshold(gray_warped_img2, 1, 255, cv2.THRESH_BINARY)

        # result = warpedResImg + warpedImage2 // used to be this line
        # result = PyramidBlender.blend_images()
        # result = PyramidBlender.blend_images_lowres(warpedResImg, warpedImage2, mask_binary)

        # invert the mask 
        mask_inv = cv2.bitwise_not(mask_binary)

        background = cv2.bitwise_and(warpedResImg, warpedResImg, mask=mask_inv)

        result = cv2.add(background, warpedImage2)
